          ...
          <div class="col-sm-7">
            @if ($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
              <div class="crowdfunding-info">
                  <span class="have-text">已筹到：</span><br>
                  <span class="total-amount">
                      <span class="symbol">￥</span>
                      {{ $product->crowdfunding->total_amount }}
                  </span>
                  <div class="progress">
                      <div class="progress-bar" role="progressbar" aria-valuenow="{{$product->crowdfunding->percent}}" aria-valuemin="0" aria-valuemax="100" style="width: {{$product->crowdfunding->percent}}%;">
                        {{$product->crowdfunding->percent}}%
                      </div>
                  </div>
            ...
            <div class="skus">
              ...
              <div class="btn-group" data-toggle="buttons">
                @foreach($product->skus as $sku)
                  <label class="btn btn-default sku-btn" data-placement="bottom" data-price="{{ $sku->price }}" data-stock="{{ $sku->stock }}" data-toggle="tooltip" title="{{ $sku->description }}">
            ...
            <div role="tabpanel" class="tab-pane" id="product-reviews-tab">
                <table class="table">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td class="product-info">{{ $review->order->user->name }}</td>
                      <td>{{$review->productSku->title}}</td>
                        <td class="vertical-middle">
                            <span class="rating-star-yes">{{ str_repeat('★', $review->rating) }}</span><span class="rating-star-no">{{ str_repeat('☆', 5 - $review->rating) }}</span>
                        </td>
                      <td>{{$review->review}}</td>
...
<script>
  $(document).ready(function() {
    ...
    $(".btn-favor").click(function() {
      axios.post('{{ route("user.favorite.product", ["product" => $product->id]) }}').then(function() {
    ...
    $(".btn-disfavor").click(function() {
      axios.delete('{{ route("user.delfavorite.product", ["product" => $product->id]) }}').then(function() {
